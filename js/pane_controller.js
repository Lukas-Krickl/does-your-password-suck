...
const paneControllerModule = (function () {
    ...
    function showResultPane(length, length_ok, letters_ok, numeric_ok, special_chars_ok, words_ok, not_leaked) {
        ...
        if ((length_ok && letters_ok && numeric_ok && special_chars_ok && not_leaked)
            ...
            resultText.innerHTML = "Secure Password!"
        } else {
            resultIcon.src = "icons/weak-password.svg"
            resultText.innerHTML = "Weak Password!"
        }
    ...
    function setWordsText(words_ok) {
        if (words_ok) {
            wordsText.innerHTML = "Does not contain words of a dictionary."
        } else {
            wordsText.innerHTML = "Contains words of a dictionar!"
        }
    }

    function setNotLeakedText(not_leaked) {
        if (not_leaked) {
            leakedText.innerHTML = "Password not leaked."
        } else {
            leakedText.innerHTML = "Password was already leaked!"
        }
    }
